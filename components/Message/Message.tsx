...
const Message = (props) => {
        ...
        useEffect(() => {
                ...
                const decryptMessage = async () => {
                        ...
                        stringToUint8Array(user?.publicKey), myKey);
                        ...
                        <View style={styles.row}>
                                ...
                                {!!decryptedContent && (
                                        <Text style={{ color: isMe ? 'black' : 'white' }}>
                                                {isDeleted ? "Message Deleted" : decryptedContent}</Text>
                                )}

                                {isMe && !!message.status && message.status !== "SENT" && (
                                        <Ionicons name={message.status === 'DELIVERED' ? 'checkmark' : 'checkmark-done'}
                                                size={16} color='gray' style={{ marginHorizontal: 5 }} />)}
                        </View>
                </Pressable>
        );
};
